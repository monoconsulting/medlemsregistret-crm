...
const MUNICIPALITY = 'Arboga';
const BASE_URL = 'https://www.forening.arboga.se/';
...
async function scrapeAssociation(
  ...
  if (listData.detailLink) {
    ...
    try {
      ...
      // Recovery: try to go back if we navigated away
      if (detailPageVisited) {
        try {
          ...
          logger.info(`Failed to go back after error: ${backError}`);
        }
      }
    }
  }
