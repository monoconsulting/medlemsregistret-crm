...
async function extractDetailPageData(page: Page, associationName: string): Promise<{
  ...
  try {
    ...
    // Left table: first header cell equals association name
    // Right table: header cell contains "Kontaktperson" or "Contact"
    ...
    if (leftTable) {
      ...
      // Note: association.phone remains null unless there's a clear association-level phone row
      // Contact person phones go into contacts, not association.phone
    }
    ...
    // This is typically the 3rd table (after association and contact tables)
    // It contains rows like Founded, Financial year begins, National organisation, Activity, Summary

    // First try to find it by heading
    ...
    for (let i = 0; i < headingCount; i++) {
      ...
      if (headingText && /övrig information|other information/i.test(headingText)) {
        ...
        if (nextTableCount > 0) {
          ...
          // Process special fields (same as before)
    ...
    // If no heading found, check remaining tables (tables that are not left or right)
    // Look for a table with rows like "Founded", "Financial year begins", "Activity", "Summary"
    ...
    // Scope to main content, exclude footer, nav, banners
    // Look for paragraphs in main content, exclude "Logga in", "Skriv ut", etc.
    try {
...
async function scrapeAssociation(
  ...
  if (listData.detailLink) {
    try {
      ...
      await page.goto(listData.detailLink);
      await randomDelay();
