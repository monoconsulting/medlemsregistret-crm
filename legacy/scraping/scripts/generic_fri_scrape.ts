...
if (args.length < 2) {
  console.error('Usage: npx tsx generic_fri_scrape.ts <municipality> <url>');
  console.error('Example: npx tsx generic_fri_scrape.ts Halmstad https://fri.halmstad.se/forening/');
  process.exit(1);
}
...
// Get paths from environment (with new naming format)
...
// Label normalization mapping (from FRI_SCRAPING_GUIDES.md)
  ...
  try {
    ...
    if (bodyText) {
      ...
      if (match) {
        ...
        log.info(`Page info detected: ${current}/${total}`);
        return { current, total };
      }
    }
  } catch (error) {
    log.info(`Warning: Could not extract page info: ${error}`);
  }
...
async function goToNextPage(page: Page, currentPage: number): Promise<boolean> {
  try {
    ...
    if (newPageInfo.current === currentPage) {
      log.info(`Warning: Page number did not change after clicking Next. Stopping pagination.`);
      return false;
    }
...
async function extractListRowData(page: Page, row: any, rowIndex: number): Promise<{
  ...
  if (cellCount >= 1) {
    ...
    if (detailLink && !detailLink.startsWith('http')) {
      detailLink = new URL(detailLink, normalizedBaseUrl).href;
    }
  }
  ...
  if (cellCount >= 4) {
    ...
    if (homepageLinkCount > 0) {
      ...
      if (homepage && !homepage.startsWith('http')) {
        homepage = new URL(homepage, normalizedBaseUrl).href;
      }
    }
  }
...
async function extractTwoColumnTable(page: Page, table: any): Promise<Record<string, any>> {
  ...
  try {
    ...
    log.info(`Error extracting two-column table: ${error}`);
  }
...
async function extractContactPerson(page: Page, rightTable: any): Promise<{
  ...
  try {
    ...
    // If mobile exists, use it; otherwise if work exists, use it; otherwise use home
    if (phones.mobile) {
    ...
    log.info(`Error extracting contact person: ${error}`);
  }
...
async function extractDetailPageData(page: Page, associationName: string): Promise<{
  ...
  try {
    ...
    log.info(`Found ${tableCount} tables on detail page for "${associationName}"`);
    ...
    for (let i = 0; i < tableCount; i++) {
      ...
      if (headerText === associationName) {
        leftTable = table;
        log.info(`Identified left table (association info) at index ${i}`);
      } else if (headerText && /kontaktperson|contact/i.test(headerText)) {
        rightTable = table;
        log.info(`Identified right table (contact person) at index ${i}`);
      }
    }
    ...
    if (leftTable) {
      ...
      log.info(`Extracted ${Object.keys(leftData).length} fields from left table`);
      ...
      if (leftData.address_raw) {
        ...
        log.info(`Parsed address: street="${result.street_address}", postal="${result.postal_code}", city="${result.city}"`);
      }
    ...
    if (rightTable) {
      ...
      if (contactData.contact_person_name) {
        ...
        log.info(`Extracted contact: ${contactData.contact_person_name} (email: ${contactData.contact_person_email}, phone: ${contactData.contact_person_phone})`);
      }
    }
    ...
    for (let i = 0; i < headingCount; i++) {
      ...
      if (headingText && /övrig information|other information/i.test(headingText)) {
        log.info(`Found "Övrig information" section via heading`);
        ...
        if (nextTableCount > 0) {
          ...
          log.info(`Extracted ${Object.keys(sectionData).length} fields from Övrig information section`);
      ...
      for (let i = 0; i < tableCount; i++) {
        ...
        if (firstCellCount >= 2) {
          ...
          if (firstLabel && /founded|bildad|financial year|verksamhetsår|activity|verksamhet|summary|sammanfattning/i.test(firstLabel)) {
            log.info(`Found "Övrig information" section as table ${i} (no heading)`);
    ...
    try {
      ...
      if (mainContentCount > 0) {
        ...
        for (let i = 0; i < paragraphCount; i++) {
          ...
          // Exclude utility text (per FRI_SCRAPING_GUIDES.md)
        ...
        if (freeTextParts.length > 0 && !result.free_text) {
          ...
          log.info(`Extracted free text (${result.free_text.length} chars)`);
        }
      }
    } catch (error) {
      log.info(`Could not extract free text from main content: ${error}`);
    }
    ...
    log.info(`Error extracting detail page data: ${error}`);
  }
...
async function scrapeAssociation(
  ...
  const record: AssociationRecord = {
    ...
    association: {
      ...
      detail_url: listData.detailLink || normalizedBaseUrl,
      street_address: null,
      postal_code: null,
      city: null,
      email: null,
  ...
  if (listData.detailLink) {
    ...
    try {
      log.info(`Visiting detail page: ${listData.detailLink}`);
      await page.goto(listData.detailLink, { timeout: 180000 }); // Increased timeout to 180s
      await randomDelay();
      ...
      log.info(`Error visiting detail page for "${listData.name}": ${error}`);

      // Recovery: if we navigated away but failed, try to go back to the list
      if (detailPageVisited) {
        try {
          log.info(`Attempting to navigate back to list page after error...`);
          await page.goBack();
          await randomDelay(500, 1000);
          await waitForListReady(page);
        } catch (recoveryError) {
          log.info(`Failed to recover: ${recoveryError}`);
          // Last resort: reload the list page
          try {
            log.info(`Last resort: reloading list page...`);
            ...
            log.info(`Critical error: could not reload list page: ${reloadError}`);
            throw reloadError;
          }
        }
      }
...
async function scrapePage(page: Page, pageIndex: number): Promise<AssociationRecord[]> {
  ...
  try {
    ...
    log.info(`Page ${pageIndex}: Found ${rowCount} associations`);
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      if (!listData.name) {
        log.info(`Skipping row ${i} - no name found`);
        continue;
      }

      log.info(`[${pageIndex}.${i}] Processing: ${listData.name}`);
    ...
    log.info(`Error scraping page ${pageIndex}: ${error}`);
  }
...
async function main() {
  ...
  log.info(`Base URL: ${normalizedBaseUrl}`);
  log.info('='.repeat(80));
  ...
  try {
    ...
    log.info(`Navigating to ${normalizedBaseUrl}`);
    await page.goto(normalizedBaseUrl);
    await waitForListReady(page);

    // Get total pages
    const pageInfo = await getPageInfo(page);
    log.info(`Detected ${pageInfo.total} total pages`);
    ...
    while (hasMore) {
      ...
      if (pageInfoAfter.current >= pageInfoAfter.total) {
        log.info(`Reached last page (${pageInfoAfter.current}/${pageInfoAfter.total})`);
        hasMore = false;
      } else {
        hasMore = await hasNextPage(page);
        if (hasMore) {
          log.info(`Navigating to page ${currentPage + 1}...`);
          const success = await goToNextPage(page, currentPage);
          if (!success) {
            log.info('Failed to navigate to next page. Stopping.');
            hasMore = false;
          } else {
            currentPage++;
          }
        } else {
          log.info('No next page available. Stopping.');
        }
      }
    }

  } catch (error) {
    log.info(`Fatal error during scraping: ${error}`);
  } finally {
    await browser.close();
  }

  // Sanitize records for validation
  log.info('Sanitizing records for validation...');
  const sanitizedRecords = allRecords.map(r => sanitizeForValidation(r));

  // Write pretty JSON (only format as per new standard - October 2025)
  ...
  log.info(`Missing org_number: ${missingOrgNumber} (${((missingOrgNumber / totalAssociations) * 100).toFixed(1)}%)`);
  log.info(`Missing contacts: ${missingContacts} (${((missingContacts / totalAssociations) * 100).toFixed(1)}%)`);
  log.info(`Missing address: ${missingAddress} (${((missingAddress / totalAssociations) * 100).toFixed(1)}%)`);
  log.info(`Unique types: ${uniqueTypes.size}`);
  log.info(`Unique activities: ${uniqueActivities.size}`);
  log.info(`Homepage domains: ${homepageDomains.size}`);
  log.info(`\nOutput files:`);
