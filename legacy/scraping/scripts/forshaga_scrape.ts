...
const MUNICIPALITY = 'Forshaga';
const BASE_URL = 'https://forening.forshaga.se/';
const SCRAPE_RUN_ID = uuidv4();
const SCRAPED_AT = new Date().toISOString();

// Get paths from environment
const paths = getScrapingPaths();
const OUTPUT_DIR = paths.jsonDir;
const JSONL_PATH = path.join(OUTPUT_DIR, `${MUNICIPALITY.toLowerCase()}_associations_${SCRAPE_RUN_ID}_${new Date().toISOString().replace('T', '_').replace(/:/g, '-').substring(0, 16)}.jsonl`);
const JSON_PATH = path.join(OUTPUT_DIR, `${MUNICIPALITY.toLowerCase()}_associations_${SCRAPE_RUN_ID}_${new Date().toISOString().replace('T', '_').replace(/:/g, '-').substring(0, 16)}.json`);
const LOG_PATH = path.join(paths.logsDir, `${MUNICIPALITY.toLowerCase()}_log_${new Date().toISOString().substring(0, 10)}.log`);
...
async function scrapeAssociation(
  ...
  if (listData.detailLink) {
    try {
      ...
      await page.goto(listData.detailLink, { timeout: 60000 });
      await randomDelay();
...
async function main() {
  ...
  const sanitizedRecords = await sanitizeForValidation(allRecords);
  ...
  await runDatabaseImport(JSON_PATH);
