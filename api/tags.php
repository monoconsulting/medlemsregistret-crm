 ...
 * GET:
 *   - List tags.
 *   - Optional query parameter association_id returns assigned tags alongside list.
 * POST:
 *   - Create tag: { name }
...
if ($method === 'GET') {
  ...
  $associationId = isset($_GET['association_id']) ? (int)$_GET['association_id'] : 0;
  if ($associationId > 0) {
    $stmt = db()->prepare('SELECT tag_id FROM association_tags WHERE association_id = ?');
    $stmt->bind_param('i', $associationId);
    $stmt->execute();
    $assignedRes = $stmt->get_result();
    $assigned = [];
    while ($row = $assignedRes->fetch_assoc()) {
      $assigned[] = (int)$row['tag_id'];
    }
    json_out(200, ['items' => $items, 'assigned' => $assigned]);
  }
