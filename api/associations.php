...
if ($method === 'GET') {
  ...
  // ---------- Items ----------
  $sql = "SELECT DISTINCT a.id, a.name, a.municipality_id, a.type, a.status, a.email, a.phone,
...
if ($method === 'POST') {
  ...
  $municipality_id = array_key_exists('municipality_id', $b) ? ($b['municipality_id'] === null ? null : (int)$b['municipality_id']) : null;
  ...
  $stmt = db()->prepare($sql);
  $municipalityParam = $municipality_id ?? 0;
  $typeParam = $type ?? '';
  $statusParam = $status ?? '';
  $emailParam = $email ?? '';
  $phoneParam = $phone ?? '';
  ...
  foreach ($fields as $k => $_) {
    if (array_key_exists($k, $b)) {
      if ($k === 'municipality_id') {
        $set[] = 'municipality_id = NULLIF(?, 0)';
        $types .= 'i';
        $vals[] = is_null($b[$k]) ? 0 : (int)$b[$k];
        continue;
      }
      $set[] = "$k = NULLIF(?, \"\")";
      $types .= 's';
      $vals[] = is_null($b[$k]) ? '' : trim((string)$b[$k]);
    }
  }
