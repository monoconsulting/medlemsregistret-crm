...
import type { Association, AuthUser } from '@/lib/api'
...
export function EditAssociationModal({ open, onOpenChange, association, users, onSubmit, isSubmitting }: EditAssociationModalProps) {
  const form = useForm<AssociationUpdateInput>({
    ...
    defaultValues: {
      ...
      streetAddress: association.street_address ?? null,
      postalCode: association.postal_code ?? null,
      city: association.city ?? null,
      email: association.email ?? null,
      phone: association.phone ?? null,
      homepageUrl: association.website ?? null,
      activities: association.activities ?? [],
      otherInformation: typeof association.description === 'string' ? association.description : '',
      descriptionFreeText: association.description_free_text ?? '',
      notes: '',
    },
  })
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      placeholder="En aktivitet per rad (tryck Enter för ny rad)"
                      rows={5}
                      value={field.value?.join('\n') ?? ''}
                      onChange={(event) => {
                        // Split on newlines or commas, keep empty lines during editing
                      ...
                      onKeyDown={(event) => {
                        // Allow Enter key without submitting form
                        if (event.key === 'Enter') {
                  ...
                  <FormDescription>Skriv en aktivitet per rad. Tryck Enter för ny rad. Tomma rader tas bort vid sparande.</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
