...
export const api = {
  ...
  async getAssociations(filters: AssocFilters = {}): Promise<ListResponse<Association>> {
    ...
    const url = qs ? `/api/associations.php?${qs}` : '/api/associations.php'
  ...
  /**
   * Fetches all tags and the selection for a specific association.
   *
   * Args:
   *   associationId: number
   *
   * Returns:
   *   Promise<{ items: Tag[]; assigned: number[] }>
   */
  async getTagsForAssociation(
    associationId: number,
  ): Promise<{ items: Tag[]; assigned: number[] }> {
    const res = await jsonFetch(
      `/api/tags.php?association_id=${encodeURIComponent(String(associationId))}`,
      { method: 'GET' },
    )
    return {
      items: (res.items ?? []) as Tag[],
      assigned: Array.isArray(res.assigned) ? (res.assigned as number[]) : [],
    }
  },
